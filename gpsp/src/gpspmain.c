...
void showFPS( u32 showfps )
	{
	gp2x_fps_debug = showfps;
	}
#endif
...
/*
  cheats[0].cheat_active = 1;//preferences.cheat1;
  cheats[1].cheat_active = 1; //preferences.cheat2;
  cheats[2].cheat_active = 1; //preferences.cheat3;
  cheats[3].cheat_active = 1; //preferences.cheat4;
  cheats[4].cheat_active = 1; //preferences.cheat5;
  cheats[5].cheat_active = 1; //preferences.cheat6;
  cheats[6].cheat_active = 1; //preferences.cheat7;
  cheats[7].cheat_active = 1; //preferences.cheat8;*/
      
  last_frame = 0;
  current_frameskip_type = auto_frameskip;
  
  //synchronize_flag = 1; //summeli: todo!
#else
  if(rom)
  {
    if(load_gamepak(rom) == -1)
  ...
  else
  {
    //set_gba_resolution(fullscreen);
    //video_resolution_small();
    DEBUG( "Try to load a file");
    //DEBUG ONLY, NO UI USED
    if(load_file(file_ext, load_filename) == -1)
    {
    DEBUG( "Load file failed, show menu");
      menu(copy_screen());
    }
    else
    {
...
void synchronize()
{
  ...
  //summeli: TODO: add FPS debugs in future
